...
class TestStaticRoutes:
    def test_index(self, client):
        ...
        assert b"SAMPLE CHOPPAH" in resp.data
...
class TestExport:
    ...
    def test_export_gap_duration_clamped(self, client):
        """gap_duration should be clamped between 0 and 10."""
        # Negative gap is clamped to 0 — still no valid regions though
        resp = client.post("/api/export", json={
            "ordered_regions": [{"type": "url", "url": "https://evil.com", "start": 0, "end": 1}],
            "gap_duration": -5,
        })
        assert resp.status_code == 400
    ...
    @patch("server.routes.export.subprocess.run")
    @patch("server.routes.upload.subprocess.run")
    def test_export_custom_gap(self, mock_upload_run, mock_export_run, client, sample_wav):
        """Export accepts a custom gap_duration."""
        mock_upload_run.return_value = MagicMock(returncode=0)
        mock_export_run.return_value = MagicMock(returncode=0)
        upload_resp = client.post(
            "/api/upload",
...
class TestStream:
    ...
    def test_disallowed_url(self, client):
        resp = client.get("/api/stream?url=https://evil.com/audio.mp3")
        assert resp.status_code == 403

    @patch("server.routes.stream.subprocess.run")
    def test_stream_success(self, mock_run, client, sample_wav, tmp_path):
        """Stream returns cached audio when yt-dlp succeeds."""
        from server.routes.stream import CACHE_DIR
        import hashlib
        ...
        cached_file = CACHE_DIR / f"{tag}.webm"
        cached_file.write_bytes(sample_wav)

        mock_run.return_value = MagicMock(returncode=0)
        resp = client.get(f"/api/stream?url={url}")
        assert resp.status_code == 200
        # Consume response to release file handle (needed on Windows)
        _ = resp.data
    ...
    @patch("server.routes.stream.subprocess.run")
    def test_stream_download_failure(self, mock_run, client):
        mock_run.return_value = MagicMock(returncode=1, stdout="", stderr="error")
        resp = client.get("/api/stream?url=https://www.youtube.com/watch?v=nope")
        assert resp.status_code == 502

    @patch("server.routes.stream.subprocess.run")
    def test_stream_soundcloud(self, mock_run, client):
        """SoundCloud URLs are allowed through the stream endpoint."""
        mock_run.return_value = MagicMock(returncode=1, stdout="", stderr="not available")
        resp = client.get("/api/stream?url=https://soundcloud.com/user/track")
        # Will fail with 502 (yt-dlp mocked to fail) but NOT 403
        assert resp.status_code != 403

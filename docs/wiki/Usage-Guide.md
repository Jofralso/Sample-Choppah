...
The choppah resolves the URL, pulls the metadata (title, thumbnail, duration), and loads the player. No audio is downloaded yet — that happens at export time.
...
3. Concatenates all slices in queue order with **1-second silence gaps** between each
4. Packages it as your chosen format
5. Sends you the download

### What You Get

A single audio file with your chops played back-to-back, separated by 1-second gaps. Load it into Serato, rekordbox, Traktor, burn it to a dubplate on vinyl, throw it on your SP-404 — it's ready to rock.

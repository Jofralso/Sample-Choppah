...
import { $, $regionCount, $exportControls, $queueList, $queueEmpty, $timelineStrip, $sourceCount, $gapDuration } from './dom.js';
...
export function renderQueue() {
  ...
  $queueList.innerHTML = state.exportQueue.map((q, i) => {
    ...
    const gap = parseFloat($gapDuration?.value) || 1;
    const gapLabel = gap > 0 ? `── ${gap}s ──` : '── no gap ──';
    return `
      ${i > 0 ? `<div class="queue-sep">${gapLabel}</div>` : ''}
...
export function renderTimeline() {
  ...
  readyQueue.forEach((q, i) => {
    ...
    total += dur;
    if (i < readyQueue.length - 1) total += gap;
  });
